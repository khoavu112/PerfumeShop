...
public class PictureDao extends AbstractDAO {
	...
	public int edit(String fileName, int id) {
		int result = 0;
		con = DBConnectionUtil.getConnection();
		String sql = "UPDATE pic_per SET picture = ? WHERE pic_per.per_id = ?";
	...
	public List<Picture> viewsID(int idPF) {
		...
		con = DBConnectionUtil.getConnection();
		String sql = "SELECT * FROM pic_per WHERE per_id = ? order by pic_per.id LIMIT 1";
