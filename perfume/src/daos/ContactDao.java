...
public class ContactDao extends AbstractDAO {
		...
		try {
			...
			while (rs.next()) {
				Contact contact = new Contact(rs.getInt("id"), rs.getString("name"), rs.getString("email"),
						rs.getString("phone"), rs.getString("message"));
				list.add(contact);
			}
	...
	public List<Contact> findByNameCustomer(String nameCustomer) {
		List<Contact> list = new ArrayList<Contact>();
		con = DBConnectionUtil.getConnection();
		String sql = "SELECT * FROM contact WHERE name LIKE ? order by id DESC";
		try {
