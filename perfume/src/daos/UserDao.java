...
public class UserDao extends AbstractDAO {
	...
	public int edit2(User user) {
		int result = 0;
		con = DBConnectionUtil.getConnection();
		String sql = "UPDATE user SET fullname = ?, username = ?,money = ? ,cat_user = ? WHERE id = ?";
		
		try {
			pst = con.prepareStatement(sql);
			pst.setString(1, user.getFullname());
			pst.setString(2, user.getUsername());
			pst.setString(3, user.getMoney());
	...
	public List<User> searchName(String nameUser) {
		...
		String sql = "SELECT user.*, catUser.id AS catID,catUser.catuser AS catUS FROM user"
				+ " INNER JOIN cat_user AS catUser ON user.cat_user = catUser.id"
				+ " WHERE username LIKE ? order by id DESC LIMIT 0,5";
	...
	public List<User> searchNameAndCat(String nameUser, int catUS) {
		List<User> searchName = new ArrayList<User>();
		con = DBConnectionUtil.getConnection();
		String sql = "SELECT user.*, catUser.id AS catID,catUser.catuser AS catUS FROM user"
				+ " INNER JOIN cat_user AS catUser ON user.cat_user = catUser.id"
				+ " WHERE username LIKE ? AND catUser.id = ? order by id DESC LIMIT 0,5";
	...
	public boolean checkTrung(String username) {
		...
		try {
			...
			rs = st.executeQuery(sql);
			while(rs.next()) {
				String usenameForDB = rs.getString("username");	
				if(username.equalsIgnoreCase(usenameForDB)) {
					ck = false;
					break;
				}
			}
