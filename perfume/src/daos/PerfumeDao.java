...
public class PerfumeDao extends AbstractDAO {
	...
	public int getCount() {
		int result = 0;
		con = DBConnectionUtil.getConnection();
		String sql = "SELECT COUNT(id) AS count FROM perfumes";
		try {
			st = con.createStatement();
			rs = st.executeQuery(sql);
			if(rs.next()) {
	...
	public List<Integer> getIDPerfumeByCatID(int catID) {
		List<Integer> list = new ArrayList<Integer>();
		con = DBConnectionUtil.getConnection();
		String sql = "SELECT id from perfumes WHERE cat_perfume = ?";
		try {
	...
	public List<Perfume> search(String namePro) {
		List<Perfume> list = new ArrayList<Perfume>();
		con = DBConnectionUtil.getConnection();
		String sql = "SELECT perfumes.*,cat.catperfume AS catPF,cat.id AS catID" + " FROM perfumes"
				+ " INNER JOIN cat_perfume AS cat ON perfumes.cat_perfume = cat.id" + " where perfumes.name_per LIKE ? ORDER BY perfumes.id DESC";
	...
	public int editSoluongTonKho(int idPer,int tonKho) {
		int result = 0;
		con = DBConnectionUtil.getConnection();
		String sql = "UPDATE `perfumes` SET `amount` = ? WHERE `perfumes`.`id` = ?";
		try {
			pst = con.prepareStatement(sql);
			pst.setInt(1, tonKho);
			pst.setInt(2, idPer);
	...
	public List<Perfume> findAllEvaluate2(int offsetAdmin) {
		List<Perfume> listEVA = new ArrayList<Perfume>();
		con = DBConnectionUtil.getConnection();
		String sql = "SELECT perfumes.*,cat.catperfume AS catPF,cat.id AS catID" + " FROM perfumes"
				+ " INNER JOIN cat_perfume AS cat ON perfumes.cat_perfume = cat.id" + " ORDER BY perfumes.id DESC"
	...
	public List<Perfume> searchCatID(int id, String namePer) {
		List<Perfume> findCatID = new ArrayList<Perfume>();
		con = DBConnectionUtil.getConnection();
		String sql = "SELECT perfumes.*,cat_perfume.*"
		+ " FROM `perfumes`" 
		+ " INNER JOIN cat_perfume ON perfumes.cat_perfume = cat_perfume.id" 
		+ " WHERE cat_perfume = ? AND perfumes.name_per LIKE ?";
		try {
			pst = con.prepareStatement(sql);
			pst.setInt(1, id);
			pst.setString(2,"%"+namePer+"%");
			rs = pst.executeQuery();
			while(rs.next()) {
				Perfume perCat = new Perfume(rs.getInt("id"),
						rs.getString("name_per"),
